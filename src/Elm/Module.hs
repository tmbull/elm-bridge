...
makeModuleContentWithOutDecAndEnc ::
     (ETypeDef -> ETypeDef) -> [DefineElm] -> String
...
recAlterType f td =
  case td of
    ...
    ETypeSum s ->
      ETypeSum (s {es_constructors = map alterTypes (es_constructors s)})
  where
    alterTypes :: SumTypeConstructor -> SumTypeConstructor
    alterTypes (STC cn dn s) =
      STC cn dn $
      case s of
...
newtypeAliases nts (ETypeAlias e) =
  ETypeAlias $
  if et_name (ea_name e) `elem` nts
...
defaultTypeAlterations t =
  case t of
    ...
    ETyApp (ETyApp (ETyCon (ETCon "HashMap")) k) v ->
      checkMap (defaultTypeAlterations k) (defaultTypeAlterations v)
    ETyApp (ETyApp (ETyCon (ETCon "THashMap")) k) v ->
      checkMap (defaultTypeAlterations k) (defaultTypeAlterations v)
  ...
  where
    ...
    tc = ETyCon . ETCon
    checkMap k v
      | isString k = ETyApp (ETyApp (tc "Dict") k) v
      | otherwise = ETyApp (tc "List") (ETyApp (ETyApp (ETyTuple 2) k) v)
    checkSet s
      | isComparable s = ETyApp (ETyCon (ETCon "Set")) s
